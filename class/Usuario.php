...
class Usuario extends Crud{
    ...
    public function validar_cadastro()
    {
        if (!preg_match("/^[A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ'\s]+$/", $this->nome)) {
            $this->erro["erro_nome "]= "Por favor informe um nome valido!";
            }
        if(!filter_var($this->email, FILTER_VALIDATE_EMAIL)){
            $this->erro["erro_email"] = "Formato de e-mail invalido";
        }
        if(srtlen($this->.senha) < 6){
    ...
    public function insert(){
        ...
        if(!$usuario){
            ...
            $sql = DB::prepare($sql);
            return $sql->execute(array($this->nome, $this->email, $this->senha_cripto, $this->recupera_senha, $this->token, $this->codigo_confirmacao, $this->status, $this->data_cadastro,));
        }else{
            $this->erro["erro_geral"] = "Usuario ja cadastrado!!",
        }
    }
